...
//!
//! The memory layout follows `CartIPCData` in `src/os/cart/os_abi.zig`, which
//! PR #156 rearranged: the framebuffers now open the block and every other
//! field sits behind the tracy ring. The simulator kept the old layout, so the
//! two backends no longer share offsets.

use crate::audio::{Shape, ToneLen};
use crate::gfx::{self, Rect, HEIGHT, WIDTH};
use crate::platform::ipc::FRAMEBUFFER_BYTES;
use core::sync::atomic::{AtomicBool, Ordering};

/// Base of `CartIPCData`, the start of `process_ram` (`src/os/cart/os_abi.zig`).
const BASE: usize = 0x2002_0000;

// Offsets from the field order in `CartIPCData`, which is the source of truth;
// the address comments beside each field there agree with these.
const FRAMEBUFFER: usize = 0x0; // two of them, column-major u16, 0xA000 each
const TRACE_BUF: usize = 0x15000; // 128 bytes, after the 4 KiB tracy ring
const NEOPIXELS: usize = 0x15080; // [{g, r, b}; 5]
const CONTROLS: usize = 0x15090; // u16
const LIGHT_LEVEL: usize = 0x15092; // u16
const USER_LED: usize = 0x15094; // bool
const BATTERY_LEVEL: usize = 0x15096; // u16
const DIRTY_RECT: usize = 0x15098; // Rect8: min_x, min_y, max_x, max_y as u8
const TONE_FREQ: usize = 0x1509C; // f32
const TONE_DURATION: usize = 0x150A0; // f32 seconds
const TONE_VOLUME: usize = 0x150A4; // f32
const TONE_FLAGS: usize = 0x150A8; // u32
const GLOBAL_VOLUME: usize = 0x150AC; // f32
// Then the tracy read position, write control word and spinlock on their own
// 16-byte granules, and the vsync and clear-color fields, through 0x150EC.
// We never touch any of them.
const IPC_END: usize = BASE + 0x150EC;

const _: () = assert!(
    TRACE_BUF == FRAMEBUFFER + 2 * FRAMEBUFFER_BYTES + 0x1000,
    "the tracy ring sits between the framebuffers and the trace buffer"
);
...
pub fn present(buf: &[u16; WIDTH * HEIGHT], dirty: Option<Rect>) {
    ...
    // `Rect8` in `src/os/cart/api.zig`: min and max corners, max exclusive,
    // one byte each. `r` is clipped to the screen, so every value fits.
    write_at::<u8>(DIRTY_RECT, r.x as u8);
    write_at::<u8>(DIRTY_RECT + 1, r.y as u8);
    write_at::<u8>(DIRTY_RECT + 2, (r.x + r.w) as u8);
    write_at::<u8>(DIRTY_RECT + 3, (r.y + r.h) as u8);

    // The pixels and the rect have to land before Core 0 sees the message;
    // `fifo_write` carries the barrier that guarantees it.
    // `PresentFlags` in `src/os/cart/os_abi.zig`: bit 0 = buffer index (always
    // 0 here), bit 1 = dirty rect valid, bit 2 = vsync fields updated, bit 3 =
    // clear frame.
...
pub fn controls() -> u16 {
    read_at::<u16>(CONTROLS)
}

pub fn light_level() -> u16 {
    read_at::<u16>(LIGHT_LEVEL) & 0x0fff
}

pub fn battery_level() -> u16 {
    read_at::<u16>(BATTERY_LEVEL) & 0x0fff
}
...
/// Park. Holding Start+Select for half a second makes the OS halt Core 1 and
/// return to the menu, so a panicked cart is recoverable without a power cycle.
pub fn abort() -> ! {
    loop {
